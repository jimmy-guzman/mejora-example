...
export class AsyncUtils {
  ...
  static async retry<T>(fn: () => Promise<T>, maxAttempts, delay: number = 1000): Promise<T> {
  ...
  static async debounce<T>(fn: () => Promise<T>, delay: number): Promise<string> {
    const result: T = await fn();
    await this.delay(delay);
    return result; // Wrong return type
  }

  static async waitFor(condition: () => boolean, timeout: number = 5000): Promise<boolean> {
